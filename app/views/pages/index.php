...
<ul class="inline-block-list">
	<?php foreach($providers as $provider => $url): ?>
		<li class="panel panel-default provider-panel">
			<a href="<?= $url ?>" class="<?= Str::lower($provider) ?>" target="_blank">
				<div class="panel-body icon-<?= Str::lower($provider) ?>"></div>
				<div class="panel-footer"><?= $provider ?></div>
			</a>
		</li>
	<?php endforeach ?>
</ul>

<a href="<?= URL::to('docs') ?>" class="">Full list of supported OAuth providers</a>
...
<p>Your Laravel config will need a little editing to load the OAuth Facde and service provider, so lets do that now</p>
...
<pre>php artisan config:publish thomaswelton/laravel-facebook</pre>
